...
Gui, Add, Button, gShowAbout        x330 y130 w160 h30, % Chr(0x2139)   " About RPCL"           ;ℹ️
...
;Row 5 buttons
Gui, Add, Button, gKillRPCS3        x330 y250 w160 h30, % Chr(0x1F480)   " Kill RPCS3"          ;💀
...
RunRPCS3:
    ...
    ;MsgBox, Reading from: %IniFile%
    SB_SetText("Reading from: " . IniFile)

    ; Read the path once
    IniRead, appPath, %IniFile%, RPCS3, Path

    ;MsgBox, ErrorLevel: %ErrorLevel%`nPath read: %appPath%
    SB_SetText("ErrorLevel: " . ErrorLevel . " | Path read: " . appPath)

    ; Check if reading was successful
    if (ErrorLevel)
    {
        SB_SetText("ERROR: Could not read the path from " . IniFile)
        Log("ERROR", "Could not read the path from " . IniFile)
        Return
    }
...
MsgBox, 64, About RPCL, %aboutText%
}
