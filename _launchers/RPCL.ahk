...
Gui, Add, Button, gRunGame          x330 y50 w160 h30, % Chr(0x1F3AE)   " Run Game"  	        ;🎮
...
;Gui, Add, Button, gOpenDiscord      x330 y210 w160 h30, % Chr(0x1F310)  " RPCS3 Discord"        ;🌐

;Row 5 buttons
Gui, Add, Button, gKillRPCS3        x330 y250 w160 h30, % Chr(0x2716)    " Kill RPCS3"           ;✖
...
Gosub, UpdatePriority  ;Force one immediate update
...
Gui, Show, w520 h250, RPCL3, RPCS3 Priority Control Launcher
...
Menu, Tray, Tip, RPCS3 Launcher - Running
...
if (rpcs3Path = "") {
    SB_SetText("ERROR: RPCS3 path not selected or invalid.")
    Log("ERROR", "RPCS3 path not selected or invalid.")
...
ShowVersion() {
    rpcs3Path := GetRPCS3Path()
    if (rpcs3Path = "") {
        SB_SetText("RPCS3 path not selected or invalid!")
        Log("ERROR", "RPCS3 path not selected or invalid!")
        return
    }
	...
	versionOutput := GetCommandOutput("%rpcs3Exe% --version")
    if (versionOutput = "") {
		return
    }
...
GetRPCS3Path() {
    ...
    if !FileExist(iniFile) {
        Log("ERROR", "Missing config.ini when calling GetRPCS3Path()")
        return ""
    }

    IniRead, path, %iniFile%, RPCS3, Path
    if (ErrorLevel) {
        Log("ERROR", "Could not read [RPCS3] Path from config.ini")
        return ""
    }
    ...
    Log("ERROR", "Invalid or non-existent path in config: " . path)
    return ""
}
...
RunRPCS3:
    ...
    if !FileExist(IniFile)
    {
        SB_SetText("ERROR: Missing config.ini file! Set RPCS3 Path first!")
        Log("ERROR", "Missing config.ini file! Set RPCS3 Path first!: " . IniFile)
        Return
    }
    ...
    if (ErrorLevel)
    {
        SB_SetText("ERROR: Could not read the path from config.ini")
...
aboutText := "RPCS3 Process Priority Launcher`nVersion: " version "`n" Chr(169) " " A_YYYY " Philip"
