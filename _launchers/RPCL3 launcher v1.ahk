...
;Twitch RR_357000
;(Process) Priority Control Launcher for RPCS3 (RPCL3)
...
;Set global variable for the .exe
global rpcs3Path := ""
global rpcs3Exe := ""

InitRPCS3Vars() {
    global rpcs3Path, rpcs3Exe
    IniRead, rpcs3Path, rpcs3.ini, RPCS3, Path,
    if (rpcs3Path = "") {
        MsgBox, 16, Error, Could not read RPCS3 path from INI.
        return false
    }
    SplitPath, rpcs3Path, rpcs3Exe
    return true
...
;-----------------------------------------------------------------------------------------------------------------------
;Windows system tray menu setup
...
;-----------------------------------------------------------------------------------------------------------------------
;This function captures the output of rpcs3.exe --help and shows it in a scrollable window:
ShowHelp:
...
rpcs3Path := GetRPCS3Path() {
    settingsFile := A_ScriptDir . "\rpcs3_path.txt"
    if FileExist(settingsFile) {
        FileRead, path, %settingsFile%
        if FileExist(path)
            return path
    }

    FileSelectFile, path,,, Select RPCS3 Executable, Executable Files (*.exe)
    if !path or !FileExist(path)
        return ""

    FileDelete, %settingsFile%
    FileAppend, %path%, %settingsFile%
    return path
}
...
if (!helpOutput) {
    MsgBox, 48, Error, Failed to run `rpcs3.exe --help`. Ensure it’s in your PATH or specify the full path.
    return
}
...
ShowVersion() {
    ...
    if (ProcessExist) {
        MsgBox, 0, RPCS3 is running, RPCS3 is currently running. Kill it to show version info?
        Log("INFO", "RPCS3 is currently running. Kill it to show version info?")
        IfMsgBox, No
            return
        Process, Close, %rpcs3Exe%
        Sleep, 500
    ...
    ; Optional: display version output
    MsgBox, %versionOutput%
