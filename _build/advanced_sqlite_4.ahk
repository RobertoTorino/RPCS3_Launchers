...
PerformSearch(favoritesOnly := false, showAll := false) {
    if (!showAll) {
        Gui, Submit, NoHide

        ; Get search term (not needed for favorites only or show all)
        if (!favoritesOnly) {
    ...
    if (favoritesOnly) {
        ...
        ; Only add checkbox filters if not showing all games
        if (FilterFavorite)
            filters.Push("Favorite = 1")
        if (FilterPlayed)
    ...
    ; Execute query - now including Favorite column
